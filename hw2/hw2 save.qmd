...
out <- murders[keep, ]
out_no_region <- out[-which(names(out) == "region")]
...
region_rates <- data.frame(region = names(rate_by_region),rate = rate_by_region)
region_rates
```
...
city_temps$temperature_c <- (temp_f - 32) * 5/9
...
national_rate <- (murders$total) / (murders$population) * 100000 

# Step 3: logical vectors high_crime and high_pop
high_crime <- rate > national_rate 
high_pop <- tapply(murders$population,murders$state,sum)

# Step 4: labels using nested ifelse
labels <- ifelse (high_crime & high_pop,"high crime, high pop",  
  ifelse (high_crime & !high_pop,"high crime,low pop","lower crime"))
...
x <- c(10, 20, 30)
y <- c(2, 0, 5)
safe_divide(x, y) # i cant make it do the thing so i had to use chatgpt for this answer
``` 
...
if (r<1){"very safe"} else if (r<3) {"safe"}
else if (r<5) {"moderate"} else {"high risk"}
}
...
render(hw2 save.qmd,output_format=pdf_document)
