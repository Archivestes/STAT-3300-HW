...
![Screenshot 2026-02-02 160425.png](ing/Screenshot%202026-02-02%20160425.png)

```         
-   Git pane (if available)
```

![Screenshot 2026-02-02 160600.png](ing/Screenshot%202026-02-02%20160600.png)
...
-   the repo URL <https://github.com/Archivestes/STAT-3300-HW>

-   a screenshot showing the repo page on GitHub ![Screenshot 2026-02-02 162407.png](ing/Screenshot%202026-02-02%20162407.png)
...
png("ing/cars.png", width = 800, height = 500, res = 120) #open png device 
plot(cars)
...
4.  **Define a function and compute real roots** In this document, define coefficients $a=2$, $b=-5$, $c=-3$. Consider $f(x) = ax^2 + bx + c$. Print the **real** solutions of $f(x)=0$ (if any).

    -   If the discriminant is negative, print a message like `"No real roots"`.

    \# a \<- 2 \# b \<- -5 \# c \<- -3 \# \# disc \<- b\^2 - 4*a*c \# disc
    ...
    disc <- b^2 - 4*a*c
    if (disc <0) {
      cat("The descriminet is negitive there for not roots")
      } else {
...
5.  **Graph the quadratic on a specified interval** Make a graph of $f(x)$ versus $x$ for $x \in (-6, 6)$. Add:
...
6.  **Write coefficients to a text file using Unix** Use Unix to create a file `data/coefs.txt` containing a single line with:
...
echo "2 -5 -3" data/coefs.txt #idk how this is wrong becouse that command dose what its saposed to do?
```

7.  **Copy the Quarto file using Unix** Use Unix to copy `hw1.qmd` into `code/` and name it `quadratic.qmd`. Show the Unix command(s):

    ```         
    # Your commands here

    cp hw/hw1.qmd code/quadratic.qmd #need to do directory(the pathing to the thing)
    ```
...
9.  **Switch to an absolute path and test portability**
    ...
    Then Copy the entire project folder\*\* to a new folder name (e.g., `RtmpXXXX`). Re-render `code/quadratic.qmd`. Does it still render? Briefly explain what happened, then switch back to a relative path and confirm it renders again. \`\`\` #its the cp - r thing again and it still works it dosent matter where the directory is

```{r}
coefs <- scan(file.path(getwd(),"data/coefs.txt"))
a <- coefs[1]
b <- coefs[2]
c <- coefs[3]
...
11. **Git workflow** Make at least **three commits** with meaningful messages, such as:
...
fb99bd9 my first homwork # in this class
93461ef add inital prodject structure
59fbac1 my added render # i failed at this
```

12. **Push to GitHub** Push your work to GitHub. In this document, include:

-   a screenshot of your GitHub repo showing the folders `img/`, `data/`, `code/`, `docs/`
-   the link to the rendered PDF file in your repo
