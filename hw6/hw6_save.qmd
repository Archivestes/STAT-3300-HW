...
hfi_2016 <- hfi |>
  filter(year == 2016) |>
  select(countries, year, hf_score, pf_score, ef_score, region)
```
...
```{r plotss-expression-score, eval=TRUE}
plot_ss(x = pf_expression_control, y = pf_score, data = hfi_2016)
```
...
```{r plotss-expression-score-squares, eval=TRUE}
...
ggplot(hfi, aes(pf_expression_control, pf_score)) +
  geom_point() +
  geom_abline(intercept = 4.28, slope = 0.542)
```
