...
summery <- dat |>
  group_by(Gender,Race3) |> 
  summarize(avg = mean(BPSysAve,na.rm=TRUE),n=n(),se = sd(BPSysAve,na.rm=TRUE)/sqrt(n),lower = avg - 1.96*se,uper = avg +1.96*se,.groups = "drop")
# 2) Create an ordering key across genders:
#    group_by(Race3) |> summarize(overall_avg = mean(avg))
order <- summery |>
  group_by(Race3) |>
  summarize(overall_avg = mean(avg), .groups = "drop")
# 3) Use fct_reorder(Race3, overall_avg) to order categories
summery |>
  left_join(order,by = "Race3") |>
  mutate(Race3 = fct_reorder(Race3,overall_avg)) |>
  ggplot(aes(x=Race3,y=avg)) + facet_wrap(~Gender) + labs(x = "Race",y = "Average",title = "Comparing systolic blood pressure across groups",caption = "Bars represent 95% confidence intervals")
...
*(Your text here.)*
```{r}
# There are a lot more data points for white on both of the male and female age and count lists. 
#the spanish have the 2 lest of the data points
```
## 8
...
dat |>
  ...
  summarize(
    ...
    children   = mean(Age < 18,na.rm = TRUE) * 100,
    .groups = "drop"  ) |>
  arrange(median_age) |>
...
dat |>
  ...
  mutate(AgeDecade = fct_relevel(AgeDecade, sort(levels(AgeDecade)))) |>
...
*(Your text here.)*
```{r}
# Mexican has a lot more older females and males 
# White has a greater rapid increse in SBP other from the gradual increse on the Mexican side
```
## 13
...
dat |>
  ...
  summarize(avg = mean(BPSysAve,na.rm = TRUE), .groups = "drop") |>
...
dat |>
  ...
  mutate(Smoke100 = if_else(Smoke100 == 1, "Smoker", "Non-smoker")) |>
...
dat |>
  ...
  summarize(
    ...
    pct_smoke = mean(Smoke100 == 1,na.rm = TRUE) * 100,
    avg_bmi = mean(BMI,na.rm = TRUE),
    .groups = "drop"  ) |>
  mutate(
